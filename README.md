...
1. [Ausgangslage](https://www.google.com/search?q=%231-ausgangslage)
2. [Lösungsidee](https://www.google.com/search?q=%232-l%C3%B6sungsidee)
3. [Vorgehen & Artefakte](https://www.google.com/search?q=%233-vorgehen--artefakte)
1. [Understand & Define](https://www.google.com/search?q=%2331-understand--define)
2. [Sketch](https://www.google.com/search?q=%2332-sketch)
3. [Decide](https://www.google.com/search?q=%2333-decide)
4. [Prototype](https://www.google.com/search?q=%2334-prototype)
5. [Validate](https://www.google.com/search?q=%2335-validate)


4. [Erweiterungen [Optional]](https://www.google.com/search?q=%234-erweiterungen-optional)
5. [Projektorganisation [Optional]](https://www.google.com/search?q=%235-projektorganisation-optional)
6. [KI-Deklaration](https://www.google.com/search?q=%236-ki-deklaration)
7. [Anhang [Optional]](https://www.google.com/search?q=%237-anhang-optional)
...
* **Ziele:** Schaffung einer „Single Source of Truth“ für Einkäufe. Das System soll Status-Transparenz in Echtzeit bieten und Einkäufe ohne zusätzliche Kommunikationsschleifen ermöglichen.
* **Primäre Zielgruppe:** Berufstätige in WGs, Familien und Paare, die ihren Haushalt effizient führen wollen.
* **Weitere Stakeholder:** Haushaltsmitglieder
...
* **End-to-End-Ablauf (Hauptfeatures):** Registrierung -> Listenübersicht (Kollaboration) -> Artikelauswahl & Laden-Zuweisung -> Einkaufsmodus (Live-Status) -> Artikel abhaken.
...
#### 3.4.1. Entwurf (Architektur & Kernkonzept)

Basierend auf den Mockups wurden folgende Kernfeatures für die Umsetzung priorisiert:

* **Informationsarchitektur:** Globale Navigationsleiste (User-Status), Listenübersicht (Startseite), Detailansicht und Einkaufsmodus
* **User Interface Design:** Mobile-First Ansicht mit klarer, schlichter Darstellung. Wischgesten für Einhandbedienung.
* **Visuelle Kommunikation (Status):** Graue Buttons signalisieren Blockierungen durch andere Nutzer, ein Stern-Icon markiert live hinzugefügte Artikel, um Doppelkäufe zu vermeiden.

#### 3.4.2. Umsetzung (Technik & Hauptfeatures)

Die im Mockup definierten Hauptfeatures wurden mit folgendem Tech-Stack realisiert:

* **Technologie-Stack & Tools:** SvelteKit 5 (Frontend/Framework) mit Runes-Syntax (`$state`, `$derived`), Visual Studio Code, GitHub Copilot, Gemini Pro.
* **Daten persistieren (Backend):** MongoDB (NoSQL) zur Speicherung von Usern, Listen und Artikeln.
* **Feature 1: Authentifizierung & User-Status:** Modularer Aufbau mit Route `/auth`. Verwendung von `bcrypt` für Password-Hashing und HTTP-only Cookies für ein sicheres Session-Management.
* **Feature 2: Listenübersicht & Kollaboration:** Route `/listen`. Nutzer können Listen erstellen, löschen, umbenennen und andere Mitglieder per E-Mail einladen. Avatare (Initialen) zeigen die Mitglieder einer Liste an.
* **Feature 3: Artikelauswahl & Laden-Zuweisung:** Route `/listen/[id]`. Das Dropdown für die Ladenauswahl füllt sich per Autovervollständigung mit bereits verwendeten Läden.
* **Feature 4: Einkaufsmodus & Live-Sync:** Ein Live-Status zeigt an, wenn ein Nutzer aktiv einkauft. Artikel im Warenkorb werden grün markiert, um die Übersichtlichkeit während des Einkaufs zu gewährleisten.

#### 3.4.3. Zusätzlicher Aufwand (Styling & Responsiveness)

Zusätzlich zu den funktionalen Kernanforderungen wurde Aufwand in ein poliertes Nutzererlebnis investiert, das über den Basis-Prototyp hinausgeht:

* **Theme & Designsprache:** Implementierung eines modernen, minimalistischen Designs, inspiriert vom Everforest-Theme.
* **CSS & Farbpalette:** Erstellung eines globalen Stylesheets mit CSS-Variablen für eine konsistente Darstellung (Beige- und Grüntöne).
* **Accessibility:** Umsetzung eines automatischen Light- und Dark-Modes, der die Systemeinstellungen des Nutzers respektiert.
* **Layout:** Überarbeitung aller Komponenten, um ein vollständig responsives, Mobile-First-Layout für alle Endgeräte sicherzustellen.

#### 3.4.4. Iterative Improvements (Post-Usability-Test)

Nach den ersten Tests wurden gezielte Anpassungen vorgenommen, um Bedienhürden abzubauen:

* **Vereinfachte Registrierung:** Die E-Mail-Bestätigung wurde entfernt und Login/Registrierung auf separate Seiten aufgeteilt. Systemmeldungen geben klares Feedback bei Erfolg oder Fehlern.
* **Klarheit im Einkaufsmodus:** Die Detailansicht zeigt nun nur noch Artikel an, die noch nicht gekauft wurden, um den Status unmissverständlich darzustellen. Der "Einkaufswagen" wurde in eine separate Seite ausgelagert. (War zum Zeitpunkt der Tests noch nicht umgesetzt gehört aber zu den Kernfeatures)
* **Navigation-Update:** Das "LiveSync"-Branding wurde entfernt, stattdessen zeigt die Navigation nun den eingeloggten Nutzer (Initialen/Avatar) an.
* **Refactoring:** Behebung diverser Kompatibilitätsprobleme durch die konsequente Umstellung auf Svelte 5 Runes.
...
Der Mindestumfang läuft stabil. Aus den Usability-Tests ergaben sich jedoch folgende primäre Usability-Probleme und Feature-Wünsche, die in der anschliessenden Iteration adressiert wurden:

1. **Top Usability-Probleme:**
* **Registrierungsprozess fehlerhaft/unklar:** Es fehlt eine Systemmeldung, dass eine E-Mail verschickt wird. Bestätigungs-E-Mails kommen teilweise nicht an, und fehlerhafte Eingaben erzeugen keine Warnung. Der Startpunkt für neue Nutzer ist nicht intuitiv.
...
2. **Next Steps (erledigt - siehe 3.4.4):**
* **Benachrichtigungen & Vereinfachung:** Klare Statusmeldungen implementiert, Registrierungsprozess (ohne Mail-Pflicht) vereinfacht.
* **Einkaufsmodus:** Der "Einkaufswagen" wurde ausgelagert, um den Status klarer zu machen.
* **Fehlende Wunsch-Features (offen):** Mengenangaben und ein Einkaufsverlauf sind noch offen.
...
| **08** | Feature | **Läden vorgeben:** Nutzer wünschen sich eine bessere Übersicht was zu welchem Laden gehört. | Niedrig | Kategorie-System im Layout ersichtlicher machen. |
...
* **Live-Synchronisation:** Implementierung von Client-Side Polling (alle 3-5 Sek.), um Änderungen anderer Nutzer ohne Seiten-Refresh anzuzeigen.
...
4. **Fehlende Funktionen:** Vorgegebene Läden. Anzahl offener Artikel werden in der Übersicht nicht angezeigt.
